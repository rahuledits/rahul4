...
const Portfolio = ({ isDark, onThemeToggle }) => {
  ...
  return (
    <>
      ...
      {showVideo && (
        <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black transition-all duration-700 animate-fade-in">
          {/* Subtle animated gradient background */}
          <div className="pointer-events-none absolute inset-0 z-0">
            <div className="absolute inset-0 bg-gradient-to-br from-[#232526] via-[#414345] to-[#232526] animate-gradient-move opacity-80" style={{backgroundSize:'200% 200%', animation:'gradient-move 12s ease-in-out infinite'}}></div>
            <div className="absolute inset-0 bg-black/70" style={{maskImage:'radial-gradient(ellipse at center, rgba(0,0,0,0) 60%, rgba(0,0,0,0.8) 100%)'}}></div>
          </div>
          {/* Glassmorphism frosted glass effect behind video and text */}
          <div className="relative z-20 flex flex-col items-center justify-center w-full h-full">
            <div className="backdrop-blur-xl bg-white/10 rounded-3xl p-10 shadow-2xl flex flex-col items-center max-w-3xl w-full">
              {!videoEnded ? (
                <>
                  <div className="mb-6 text-3xl font-extrabold text-white drop-shadow-lg animate-fade-in-up">A divine presence fills the darkness...</div>
                  <div className="relative w-full flex justify-center">
                    <video
                      src="2.mp4"
                      autoPlay
                      controls
                      className="rounded-3xl shadow-2xl border-8 border-neutral-800 max-w-2xl w-full bg-black"
                      onEnded={() => setVideoEnded(true)}
                      style={{ boxShadow: '0 0 64px 12px #7f5af0cc, 0 0 24px 4px #fff4', background: '#000' }}
                    />
                    <div className="absolute inset-0 rounded-3xl pointer-events-none" style={{boxShadow:'0 0 80px 16px #7f5af0cc, 0 0 32px 8px #fff8'}}></div>
                  </div>
                  {/* Rahul Meena button below video */}
                  ...
                  <div className="mt-8 text-2xl font-bold text-white/80 animate-fade-in-up">Let the light guide your soul.</div>
                </>
              ) : (
                <>
                  <div className="text-5xl font-extrabold text-white mb-8 animate-fade-in-up" style={{textShadow:'0 0 24px #000'}}>Transcendence awaits...</div>
                  <div className="mt-12 flex flex-col items-center justify-center gap-6 animate-fade-in-up">
                    <div className="text-3xl font-bold text-white/90 drop-shadow-lg">Ready to create something extraordinary?</div>
                    <button className="px-10 py-4 rounded-full bg-neutral-900 text-white font-extrabold text-2xl shadow-2xl hover:scale-105 transition-all duration-300 focus:outline-none focus:ring-4 focus:ring-white/30">
                      Let's Make Magic Together
                    </button>
                  </div>
                  {/* No close button, user is enveloped in the experience! */}
                </>
              )}
            </div>
          </div>
